...
builder.declare({
  ...
  await req.authorize(req.body);
  await this.db.fns.add_denylist_address(address);
  res.sendStatus(200);
});
...
builder.declare({
  ...
  await req.authorize(req.body);
  await this.db.fns.delete_denylist_address(address);
  res.sendStatus(200);
});
...
builder.declare({
  ...
  const {continuationToken, rows} = await paginateResults({
    query: req.query,
    fetch: (size, offset) => this.db.fns.all_denylist_address(size, offset),
  });
