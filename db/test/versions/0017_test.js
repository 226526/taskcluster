...
suite(testing.suiteName(), function() {
  ...
  test('denylisted_notification table created / removed on upgrade and downgrade', async function() {
    await testing.resetDb({testDbUrl: helper.dbUrl});
    await helper.upgradeTo(PREV_VERSION);

    await helper.assertTable('denylisted_notification_entities');
    await helper.assertNoTable('denylisted_notification');

    await helper.upgradeTo(THIS_VERSION);
    await helper.assertTable('denylisted_notification');
  ...
  helper.testEntityTable({
    ...
    newTableName: 'denylisted_notification',
